...
+ (CGSize)getMessageCellSize:(RCCRMessageModel *)model withWidth:(CGFloat)width{
    RCTextMessage *textMessage = (RCTextMessage *)model.content;
    NSString *localizedMessage = textMessage.content;
    if (model.senderUserId) {
        RCUserInfo *userInfo = model.userInfo;
        NSString *userName = [userInfo.name stringByAppendingString:@"："];
        localizedMessage =[NSString stringWithFormat:@"%@%@",userName,localizedMessage];
    }
    CGSize textSize = [localizedMessage boundingRectWithSize:CGSizeMake(width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:17]} context:nil].size;
