...
- (instancetype)initWithFrame:(CGRect)frame model:(RCCRLiveModel *)model {
    ...
    if (self) {
        ...
        [self.bottomBtnContentView setFrame:CGRectMake(0, size.height - 50, size.width, 50)];
        ...
        [self.danmakuBtn setFrame:CGRectMake(size.width - 35*3 - 10*4, 10, 35, 35)];
        ...
        [self.giftBtn setFrame:CGRectMake(size.width - 35*2 - 10*3, 10, 35, 35)];
        
        if (self.model.liveMode != RCCRLiveModeHost) {
            [self.bottomBtnContentView addSubview:self.praiseBtn];
        }
        [self.praiseBtn setFrame:CGRectMake(size.width - 35 - 10*2, 10, 35, 35)];
...
- (void)presentLikeMessageAnimation:(RCChatroomLike *)likeMessage {
    UIViewController *superController = [self viewContreoller:self];
    CGRect viewRect = [UIApplication sharedApplication].keyWindow.rootViewController.view.frame;
...
- (void)onInputBarControlContentSizeChanged:(CGRect)frame withAnimationDuration:(CGFloat)duration andAnimationCurve:(UIViewAnimationCurve)curve ifKeyboardShow:(BOOL)ifKeyboardShow {
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        if (ifKeyboardShow) {
            self.inputBar.frame = CGRectMake(0, originFrame.size.height-frame.size.height-50, self.inputBar.frame.size.width, self.inputBar.frame.size.height);
        }else {
            self.inputBar.frame = CGRectMake(0, self.messageContentView.frame.size.height - 50, self.inputBar.frame.size.width, self.inputBar.frame.size.height);
        }
        [UIView commitAnimations];
    }];
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([model.content isKindOfClass:[RCTextMessage class]]) {
        CGSize size = [RCCRTextMessageCell getMessageCellSize:model withWidth:self.frame.size.width];
        height = size.height;
    }
