...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if (deleteSpace.length == 0){
        [MBProgressHUD showMessage:NSLocalizedString(@"请输入要发送的内容", nil) inView:[[UIApplication sharedApplication].windows lastObject]];
    }else {
           [self closeDanmu:nil];
    }
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if ([string isEqualToString:@""]) {
        ...
        [MBProgressHUD showMessage:NSLocalizedString(@"最多输入40个字符或20个汉字", nil) inView:[[UIApplication sharedApplication].windows lastObject]];
        return NO;
    }
    return YES;
}
