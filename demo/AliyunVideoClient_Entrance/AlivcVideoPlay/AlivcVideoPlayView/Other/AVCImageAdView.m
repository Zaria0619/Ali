...
- (instancetype)initWithImage:(UIImage *)image status:(AdsStatus) status inView:(UIView *)view{
    if (self = [super init]) {
        ...
        if (image) {
            ...
            self.image = [UIImage imageNamed:@"adsPhoto"];
...
- (UIButton *)adButton{
    if (!_adButton) {
        ...
        NSMutableAttributedString * text = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"5 %@",NSLocalizedString(@"关闭广告", nil)]];
        ...
        [text setAttributes:firstAttributes range:NSMakeRange(0,1)];
...
- (void)setCountNum:(NSInteger)countNum {
    _countNum = countNum;
    NSString * title = [NSString stringWithFormat:@"%ld %@",(long)_countNum,NSLocalizedString(@"关闭广告", nil)];
...
- (void)countDown {
    ...
    [_adButton setAttributedTitle:text forState:UIControlStateNormal];
    [_adButton setTitle:title forState:UIControlStateNormal];
