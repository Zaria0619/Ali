...
@property (nonatomic, strong) AVCAirPlayView  *switchDevice;
...
@property (nonatomic, strong) AVCAirPlayView  *closeView;
...
- (UIButton *)backButton {
    if (!_backButton) {
        ...
        [_backButton setImage:[UIImage imageNamed:@"avcBackIcon"] forState:UIControlStateNormal];
...
- (UIButton *)fullScreenButton{
    if (!_fullScreenButton) {
        ...
        [_fullScreenButton setImage:[UIImage imageNamed:@"alivc_fullScreen"] forState:UIControlStateSelected];
        [_fullScreenButton setImage:[UIImage imageNamed:@"alivc_fullScreen"] forState:UIControlStateNormal];
...
- (AVCAirPlayView *) closeView {
    ...
    if (!_closeView) {
        _closeView = [AVCAirPlayView airPlayViewWithFrame:CGRectZero image:[UIImage imageNamed:@""]];
        _closeView.backgroundColor = [UIColor colorWithRed:0.00f green:0.76f blue:0.87f alpha:1.00f];
        [_closeView setText:NSLocalizedString(@"退出投屏" , nil)];
    }
    return _closeView;
}
...
- (UISlider *)playSlider{
    if (!_playSlider) {
        ...
        //value发生变化
        [_playSlider addTarget:self.controller action:@selector(updateProgressSliderAction:) forControlEvents:UIControlEventValueChanged|UIControlEventTouchUpInside|UIControlEventTouchCancel|UIControlEventTouchUpOutside];
...
- (UIImageView *)imageView {
    if (!_imageView) {
        ...
        _airplayLabel.text = NSLocalizedString(@"正在投屏中...", nil);
...
- (AVCAirPlayView *)switchDevice {
    if (!_switchDevice) {
        ...
        _switchDevice = [AVCAirPlayView airPlayViewWithFrame:CGRectZero image: [UIImage imageNamed:@"avcTV"] ];
        _switchDevice.backgroundColor = [UIColor clearColor];
        [_switchDevice setText:NSLocalizedString(@"切换设备" , nil)];
...
- (void)releasePlayer{
    ...
    [self.nativePlayer.currentItem cancelPendingSeeks];
    [self.nativePlayer.currentItem.asset cancelLoading];
    ...
    _item = nil;
    _nativePlayer = nil;
    self.nativePlayer = [[AVPlayer alloc]init];
